...
def create_sorted_table_image(sorted_data, output_path):
    ...
    ax.set_title('Tableau trié', fontsize=16)

    plt.savefig(output_path, bbox_inches='tight', pad_inches=0.1)
    plt.close()

if __name__ == "__main__":
    # Supprimer les fichiers PNG commençant par le répertoire du script
    script_directory = os.path.dirname(os.path.abspath(__file__))
    for file_name in os.listdir(script_directory):
        if file_name.endswith('.png') and (file_name.startswith('output_') or file_name.startswith('sorted_table_')):
